class SearchSuggestionsController < ApplicationController
  def index
    ...
    @songs = Song.where('unaccent(artist) ILIKE ? OR unaccent(title) ILIKE ? OR unaccent(genre)ILIKE ?', "%#{params[:q]}%", "%#{params[:q]}%", "%#{params[:q]}%").limit(5).distinct.pluck("title || ' | ' || artist || ' | ' || genre")
